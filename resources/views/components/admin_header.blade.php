<div data-aos="fade-down" class="bg-slate-400 dark:bg-slate-500 mx-auto">
    @php
    ...
    @if ($section && $section->content)
        <div class="flex justify-center">
            <a href="{{ route('home') }}">
                <img src="{{ asset($section->content) }}" alt="Title Header" class="w-full h-auto">
            </a>
        </div>
    @endif
</div>

<header data-aos="fade-down" id="header" class=" z-50 bg-blue-500 flex-col dark:bg-teal-700 dark:backdrop-blur-lg dark:bg-opacity-50 flex justify-between items-center text-gray-800 dark:text-gray-100 transition-all duration-300 shadow-md">
    <div class="max-md:flex items-center flex justify-between w-full md:px-4 max-md:px-2 py-3">
        @auth
            <div class="relative">
        ...
        @else
            <a href="{{ route('login') }}" class="rounded-lg py-2 px-4 bg-blue-500 text-white hover:bg-blue-600 dark:bg-blue-700 dark:hover:bg-blue-600 transition-all">ورود</a>
        @endauth
        ...
        <form method="GET" class="hidden lg:flex items-center" action="{{ route('search') }}">
            ...
            @php
                $types = ['همه ی موارد' => "all"];
            @endphp
            <button type="submit" class="ml-2 rounded-l-lg bg-blue-500 text-white px-4 py-2 hover:bg-blue-600 transition-all">
                جستجو
            </button>
        </form>
    ...
    <button id="submitBtn" class="rounded-lg my-2 dark:bg-blue-600 bg-white text-gray-800 dark:text-gray-100 py-2 max-md:py-1 max-md:px-3 px-6 transition-all hover:bg-gray-200 dark:hover:bg-teal-500">
        ذخیره
    </button>
</header>
