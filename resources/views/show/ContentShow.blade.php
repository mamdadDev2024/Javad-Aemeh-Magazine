            ...
            <div class="p-5">
                ...
                <div class="flex justify-between items-center mt-2">
                    <span class="text-sm text-gray-500 dark:text-gray-400">
                        نویسنده: {{ $item->user->username }}
                    </span>
                    <span class="text-sm text-gray-500 dark:text-gray-400">
                        تعداد نظرات: {{ $item->comments->count() }}
                    </span>
                </div>
                <p class="mt-4 text-gray-700 text-right dark:text-gray-300" data-aos="fade-up">
                    {!! nl2br(e($item->body)) !!}
                </p>

                <!-- Attachments Section -->
                @if ($item->pdf)
                    <div class="mt-5 bg-gray-100 dark:bg-gray-700 p-4 rounded-lg">
                        <h3 class="text-lg font-bold text-gray-900 dark:text-gray-100">فایل‌ ضمیمه</h3>
                        <a href="{{ route('download', ["url" => $item->pdf]) }}"
                            class="text-blue-500 hover:underline dark:text-orange-300">
                            دانلود PDF
                        </a>
                    </div>
                @endif
                ...
                <div class="flex items-center justify-between mt-5">
                    ...
                    <div>
                        @isset($scopes)
                            <p class=" text-bold text-lg mr-2">سطح خبر</p>
                            @forelse ($scopes as $scope)
                                <p class=" text-green-400 underline mx-2 my-1">{{$scope->name}}</p>
                            @empty
                                <p class=" text-green-400 underline mx-2 my-1">عمومی</p>
                            @endforelse
                        @endisset
                    </div>
                </div>
