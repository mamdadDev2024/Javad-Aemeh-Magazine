...
@section('body')
    <main class="flex justify-center py-8">
        <form action="{{ route('writer.event.do.update') }}" enctype="multipart/form-data" method="POST" class="container flex flex-col gap-6 p-8 bg-white shadow-lg rounded-lg">
            @csrf
            ...
            <div>
                ...
                @enderror
                <input value="{{ old('title', $Event->title) }}" type="text" name="title" id="title" aria-describedby="error-title" class="mt-1 block w-full focus:outline-none transition-all border border-gray-300 rounded-md p-2 focus:ring focus:ring-blue-400" required>
            </div>
            <div>
                ...
                <select name="category[]" id="category" aria-describedby="error-category" multiple class="rounded-lg w-72 border p-2" required>
                    <option value="" disabled {{ $Event->categories->isEmpty() ? 'selected' : '' }}>هیچ دسته بندی</option>
            ...
            <div>
                ...
                @if ($Event->image)
                    <img src="{{ asset($Event->image) }}" alt="تصویر فعلی" class="mt-2 w-24 h-24 rounded-lg">
                @endif
            </div>
            <div>
                ...
                @enderror
                <textarea name="body" id="body" aria-describedby="error-body" class="mt-1 block w-full min-h-20 max-h-96 focus:outline-none transition-all border border-gray-300 rounded-md p-2 focus:ring focus:ring-blue-400" required>{{ old('body', $Event->body) }}</textarea>
            </div>
            <x-captcha/>
            <div>
